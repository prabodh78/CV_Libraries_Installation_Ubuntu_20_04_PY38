...
void PyNet::set_input_arrays(bp::object data_obj, bp::object labels_obj, bp::object data_dim_obj) { // Martin Kersner, 2016/04/19
  ...
  PyArrayObject* data_dim_arr = reinterpret_cast<PyArrayObject*>(data_dim_obj.ptr()); // Martin Kersner, 2016/04/19
  ...
  check_contiguous_array(data_dim_arr, "labels array", 1, 1, 2); // Martin Kersner, 2016/04/19

  if (PyArray_DIMS(data_arr)[0] != PyArray_DIMS(labels_arr)[0]) {
    throw std::runtime_error("data and labels must have the same first"
        " dimension");
  }
  if (PyArray_DIMS(data_arr)[0] != PyArray_DIMS(data_dim_arr)[0]) { // Martin Kersner, 2016/04/19
    throw std::runtime_error("data and data_dim must have the same first"
        " dimension");
  }
  ...
  md_layer->Reset(static_cast<float*>(PyArray_DATA(data_arr)),
                  ...
                  static_cast<float*>(PyArray_DATA(data_dim_arr)), // Martin Kersner, 2016/04/19
