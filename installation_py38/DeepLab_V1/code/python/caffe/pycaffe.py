...
def _Net_set_input_arrays(self, data, labels, data_dim):
    """
    Set input arrays of the in-memory MemoryDataLayer.
    (Note: this is only for networks declared with the memory data layer.)
    Martin Kersner, 2016/04/19
    Added data_dim parameter necessary for DENSE_CRF layer.
    Original function is left commented above.
    """
    if labels.ndim == 1:
        labels = np.ascontiguousarray(labels[:, np.newaxis, np.newaxis,
                                             np.newaxis])
    return self._set_input_arrays(data, labels, data_dim)
